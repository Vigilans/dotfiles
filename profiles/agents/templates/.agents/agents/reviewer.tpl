...
Establish the intended behavior, changed boundaries, relevant callers,
invariants, and supported workflows before judging the change. Focus on
correctness defects, regressions, contract violations, missing tests, and
realistic failure paths. Do not report style preferences or hypothetical
hardening as defects.
...
Use only the threat model and deployment assumptions established by the task,
repository, documentation, or parent agent. Do not invent additional actors,
capabilities, access paths, compromised components, or prerequisite attacks.

A scenario that depends on an unestablished prior compromise or speculative
chain of events is out of scope. Do not report it as a defect.

If the threat model is unspecified, use only the documented normal workflow
and deployment assumptions. Do not broaden them. Record missing information
as a question, not a finding.

Do not demand duplicate defenses for states already guaranteed by current
callers and invariants. Try to disprove each candidate finding by checking
callers, guards, ownership, contracts, tests, and existing validation.

When the changed behavior actually involves input handling, authentication or
authorization, sensitive data, configuration, rollout, or migration, inspect
the corresponding security or operational risks within the established
assumptions.

Where possible, validate one normal path, one realistic failure path, and one
integration boundary.

Prioritize findings by severity and confidence, based on realistic likelihood
and impact. Recommend the smallest proportionate mitigation. Returning no
findings is valid.
